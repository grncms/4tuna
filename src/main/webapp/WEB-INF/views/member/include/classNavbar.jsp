...
<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
	...
	<ul class="navbar-nav ml-auto">
		...
		<!-- Nav Item - Messages -->
		<li class="nav-item dropdown no-arrow mx-1 ">
			<a class="nav-link dropdown-toggle position-relative" href="/chat" id="chatAlarm" role="button" data-bs-toggle="popover" data-bs-placement="bottom">
			 <i class="fa-solid fa-comment"></i> <!-- Counter - Messages --> <!-- <span class="badge badge-danger badge-counter">7</span> -->
		...
		<li class="nav-item dropdown no-arrow">
			<a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <span class="mr-2 d-none d-lg-inline text-gray-600 small">
					<b>${sessName}</b>
				</span> <c:if test="${teacherNy eq 1}">
					<span class="mr-2 d-none d-lg-inline text-gray-600 small">
						<font id="roleT">선생님</font>
					</span>
				</c:if> <c:if test="${teacherNy eq 0}">
					<span class="mr-2 d-none d-lg-inline text-gray-600 small">
						<font id="roleS">학생</font>
					</span>
				</c:if> <c:if test="${ctPath ne null}">
					<img class="img-profile rounded-circle" width="35px" height="35px" src="${ctPath}${ctUuidName}">
				</c:if> <c:if test="${empty ctPath}">
...
<script>
    $(function() {
        ...
        var params = {ctcsSeq : ctcsSeq, myId : ctcmSeq}; //메세지 갯수 세기위한 파라미터
        
        getMsgCount(params);
        
        client.connect({}, function() {
            console.log("header stomp connected")
            client.subscribe('/sub/topic/' + ctcsSeq + "/" + ctcmSeq, function(msg) {
